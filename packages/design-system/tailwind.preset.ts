...
/**
 * The token → Tailwind class mapping, shared between packages/design-system
 * (which owns it) and any consuming app that writes its own Tailwind classes
 * against this design system (e.g. apps/web). Single source of truth so the
 * two never drift — import this as a `presets` entry, don't copy it.
 */
export const shelfSensePreset: Pick<Config, "darkMode" | "theme"> = {
  darkMode: ["class"],
  theme: {
    extend: {
      colors: {
...
export default shelfSensePreset;
