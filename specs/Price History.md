...
- [x] ~~Given a product tracked by percentage (`tracking_mode === "percentage"`), which carries no `Batch` rows and therefore no price data, the "Price History" menu item is disabled with an explanatory tooltip (e.g. "Not tracked for % Off SKUs") — same treatment Product List already gives "Edit Stock" for these products. See Out of scope for the longer-term fix.~~ — resolved by `specs/Price History for % tracked products.md`: a %-tracked product can now carry (symbolic, price-log) `Batch` rows, so the menu item is enabled for it exactly like a unit-tracked product, falling into the ordinary empty state when it has none yet.
