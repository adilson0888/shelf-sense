...
- [ ] The chart's Y axis auto-scales to the currently visible dataset — no fixed min/max.
- [ ] The chart shows a flat, dashed reference line at the average price across all currently *visible* data points (i.e. it recomputes when a legend entry is toggled) — not a rolling/time-based average.
...
- [ ] At the bottom of the modal, a button jumps to Quick Batch Edit for that product: it closes Price History and opens Quick Batch Edit, the same way Quick Batch Edit's own "Edit product"/"Stock" buttons already jump to other modals.
- [ ] Given a product tracked by percentage (`tracking_mode === "percentage"`), which carries no `Batch` rows and therefore no price data, the "Price History" menu item is disabled with an explanatory tooltip — same treatment Product List already gives "Edit Stock" for these products. See Out of scope for the longer-term fix.
- [ ] Given a product has no priced purchases at all (every batch has a blank price, or the product has no batches), the modal shows an empty state instead of an empty chart.
...
- **New `PriceHistoryModal`** (`apps/web/src/components/`), built from the same `Modal`/`ModalHeader`/`ModalTitle`/`ModalBody`/`ModalFooter` primitives every other modal uses, with the line chart, legend, and top stat highlight described in Acceptance criteria. Wider than the codebase's usual `max-w-sm` form-modal width, since it needs room for a chart — a deliberate, noted exception.
