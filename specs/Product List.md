...
- **Footer bar**: same `border-t border-border bg-surface-0` treatment `Product Edit.md`'s bottom action bar uses, matched to its actual rendered height (64px — its `py-md` padding around `h-8` buttons, not `py-md` alone) since this bar has no buttons of its own to give it that height. No Save/Cancel here — this page has nothing to commit. Sits at the bottom of the page itself (the content area above it is `flex-1`), not immediately after the table when there are only a few rows.
