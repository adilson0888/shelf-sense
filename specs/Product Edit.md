...
- [ ] Given the user checks one or more persisted barcode rows and clicks "Search prices", when pressed, then Product Edit closes and the existing Price History-style modal opens with a live comparison search for exactly those barcode IDs. The comparison matrix uses the same loading, success, error, retry, site columns, and listing links as Price History. A selected barcode with no recorded purchase history is still searched and can return results; a barcode added only as unsaved Product Edit state is not searchable until the product is saved.
...
- **Barcodes table**: `DataTable`-shaped layout (checkbox column + Description + Code) — a real, if hand-rolled rather than `shelf-sense-ds` `DataTable`, table. Toolbar: when ≥1 row is checked, a danger "Remove" button and an outline "Search prices" button; the latter searches only the checked persisted barcodes and opens the existing Price History-style comparison modal. A "+ Add barcode" button opens `BarcodeCaptureModal` when scanning is supported, or the inline `description` + `code` form directly when it isn't (see Acceptance criteria) — the inline form itself is unchanged either way, just sometimes arrives pre-filled.
