...
- [ ] Given the Product Edit view (`Product Edit.md`, `ProductEditView.tsx`), when it renders, then a **Delete product** button (`Button variant="danger"`) appears at the bottom of the scrollable content, below the Barcodes section — reachable from every page that opens Product Edit (Product List, Inventory, Grocery List), not just Product List's own popover.
...
- **Entry points**: Product List page (`apps/web/src/pages/ProductList.tsx`), as a `PopoverItem` appended to the existing "⋯" `Popover` after **Price History**, with a divider above it and danger-red text — matching `Button`'s own `variant="danger"` convention (its doc comment already names "Delete item" as the canonical example). Also a **Delete product** `Button variant="danger"` at the bottom of Product Edit's own scrollable content (`ProductEditView.tsx`) — the same shared view opened from Product List, Inventory, and Grocery List, so this second entry point covers all three without each needing its own popover.
...
- **Deleting from Inventory's own "•••" row menu** (`Price History.md`) — that menu stays **Edit Stock** / **Price History** only; Inventory reaches delete via Product Edit's button instead (see UI requirements).
- **A delete affordance inside Quick Batch Edit** — that modal doesn't gain a delete button; use its own "Edit product" button to reach Product Edit's delete button instead.
