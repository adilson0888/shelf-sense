...
- **Gap to resolve**: the approved design drops the "i" detail affordance for `long_description` that this section used to call for — it didn't survive into what got approved and built. Flag whether that's intentional (maybe deferred to a later visual pass) or should be added back; as built, this screen doesn't show it.
 - **Components**: `Button`, `Input`, `FreshnessBadge` from `shelf-sense-ds` (all exist and are synced) — layout is custom Tailwind, not `DataTable` (a row-grouped list doesn't fit a table shape well).
...
- **Footer bar**: `shelf-sense-ds`'s `Footer` component (see `Product List.md`'s own note on it) — the "shelf·sense" wordmark, right-aligned. Nothing on this page to commit, same as everywhere else it's used.

## Non-functional

 - **Offline caching**: web needs a local cache (IndexedDB/localStorage) so "load from cache after first load" behaves consistently. Exact sync strategy (what happens to edits made offline) is bigger than this spec — see the earlier note about writing a dedicated offline-sync spec before too many features depend on it.
