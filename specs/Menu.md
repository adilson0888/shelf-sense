...
- **Routing**: `apps/web`'s `App.tsx` previously rendered `ProductListPage` directly with no router. Implementing Menu means introducing real routing (`react-router-dom`) — `/` → Products, `/settings` → Settings.
