...
- [ ] Given the user instead reaches this view via Product List's "Edit stock" **⋯** menu option (`Product List.md`), when it loads, then the identical view renders — a second entry point to the same route, no behavioral difference except where Save/Cancel return to (see below).
- [ ] Given the user reached this view via Quick Batch Edit's "Stock" button, when they click Save's "Confirm?" or Cancel, then they're returned to whichever of Inventory, Product List, or Grocery List (`Grocery List.md`, a third page that reuses the same Quick Batch Edit modal) hosted that modal — and given they instead reached it directly via Product List's "Edit stock" menu option, they're returned to Product List. Never hardcoded to Inventory regardless of where the user actually came from.
...
- [ ] Given Save is showing "Confirm?", when the user clicks it again, then all pending adds/edits/removes persist together, atomically, and the view returns to the page it was opened from (Inventory, Product List, or Grocery List — see the entry-point criteria above).
...
- [ ] Given the user clicks Cancel at any point, when pressed, then all pending changes are discarded and the view returns to the page it was opened from, same as Save, without persisting anything.
...
- **Full-screen view**, own header, reached via a **real route**: `/products/:id/stock`. **Not** wrapped in the app's `AppShell` (no hamburger/drawer here) — deliberately consistent with `Product Edit.md`'s existing chrome, applied to both by the same reasoning: a focused edit screen with unsaved pending state shouldn't invite navigating away through the drawer, and `AppShell`'s top-bar title logic (keyed to `Menu.md`'s fixed item list) has no way to represent a per-product route without new work neither page currently needs. **Header structure matches `Product Edit.md`'s exactly**: a "‹" back control (same return-to-origin destination as Cancel — see Acceptance criteria, no longer hardcoded to Inventory), a "STOCK EDIT" eyebrow label, and the product name — same visual family, different screen.
...
- **Return destination**: this route now has entry points on three different pages — Quick Batch Edit's "Stock" button, itself opened from Inventory, Product List, *or* Grocery List (`Grocery List.md` reuses the identical modal) — plus Product List's own "Edit stock" **⋯** menu option (`Product List.md`) that skips the modal entirely — so Save-confirm and Cancel can no longer assume Inventory. Whichever screen navigates in passes `state: { from: <its own path> }` on the `navigate()` call (`"/"`, `"/products"`, or `"/grocery"`), reusing the same `from`-in-router-state convention `Product Add.md`'s Add Product route already established (`AddProductLocationState`, `apps/web/src/pages/AddProduct.tsx`) rather than inventing a second one — `apps/web/src/pages/StockEdit.tsx`'s own `StockEditLocationState` mirrors it. This view reads `location.state?.from`, falling back to `/` (Inventory) only when it's genuinely missing, and navigates there on both Save-confirm and Cancel. Quick Batch Edit forwards whichever page it was itself opened from (see `Quick Batch Edit.md`'s own Non-functional) rather than assuming Inventory.
...
- ~~**Entry points beyond the Quick Batch Edit "Stock" button** — e.g. linking here from Inventory's expanded batch rows — considered, deliberately left for a future pass.~~ — resolved by `specs/Product List.md`, which added a second entry point (its "Edit stock" **⋯** menu option); both entry points now determine this view's return destination together (see Acceptance criteria / Non-functional above), rather than either one being the assumed default.
