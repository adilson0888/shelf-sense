...
- [ ] Given Product Edit is open (`specs/Product Edit.md`), when the user checks one or more persisted barcode rows, a **"Search prices"** `Button` appears beside **Remove**. Clicking it closes Product Edit and searches exactly the checked barcode IDs in the same Price History-style comparison flow. This entry point does not depend on those barcodes having existing price-history points, so the comparison matrix can be shown even when the chart history is empty. Newly added, unsaved barcode rows are excluded until Product Edit is saved.
...
- **`PriceHistoryModal`** (`apps/web/src/components/`): a single **Search prices** `Button` placed near the legend (not per-row) — reads the legend's existing visibility `Set`/state as its selection, no new selection UI. It also accepts an explicit barcode-ID selection from Product Edit, which bypasses chart visibility and remains valid when a selected barcode has no chart series. Below the chart, a results area renders one of: nothing (no search run yet this session), loading, the matrix table, or an error `Alert` — per Acceptance criteria. The matrix table is a small `DataTable` (or comparable), sites as columns, searched sub-products as rows. A cell with a found price renders as an `<a target="_blank" rel="noopener noreferrer">` around the formatted price when a `url` is present (styled as a link — brand-colored, underlined); a found price with no `url`, or "Not found", renders as plain text.
