...
- [ ] Given the modal is open, when the user taps the "Stock" button, then it navigates to the Stock Edit view for this product (`Stock Edit.md`) — no longer stubbed as of that spec landing. The navigation carries forward whichever page (Inventory, Product List, or Grocery List — see `Grocery List.md`, a third page reusing this same modal) this modal itself was opened from, so Stock Edit's Save/Cancel return there rather than defaulting to Inventory — see `Stock Edit.md`'s Non-functional.
- [ ] Given the modal is open, when the user taps the product-edit button, then it navigates to the Product Edit view for this product (`Product Edit.md`) — no longer stubbed as of that spec landing. No equivalent forwarding is needed here: this modal and Product Edit's overlay are both local state on whichever page hosts them, so Product Edit's Save/Cancel already return to the right page without extra plumbing — see `Product Edit.md`'s UI requirements.
...
- **Triggers** (two, both open the same modal):
  ...
  - **Reused across pages**: the identical modal is also opened by Product List's own long-press gesture (`Product List.md`) and Grocery List's (`Grocery List.md`), not just Inventory's — relevant here because the "Stock"/product-edit buttons below can no longer assume Inventory is the page to return to once the user backs out of Stock Edit/Product Edit (see Non-functional).
...
- **Return-destination forwarding**: since this modal opens from Inventory, Product List, or Grocery List, the "Stock" button's navigation into `Stock Edit.md`'s real route passes `state: { from: <the current page's path> }` — the same `from`-in-router-state convention `Product Add.md`'s Add Product route already established (`AddProductLocationState`) — so that view's Save/Cancel land back on the correct page instead of defaulting to Inventory. The product-edit button needs no equivalent forwarding: `Product Edit.md`'s overlay is local state on whichever page already hosts this modal, so it returns to the right page without anything extra being passed along.
