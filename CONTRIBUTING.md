...
**`shelf-sense-ds` is web-only.** It's HTML + compiled CSS. `/design-sync` and Claude Design likewise only ever apply to `apps/web`.
...
New feature → write a spec in `specs/` → for **web** features, prototype the UI in Claude Design against the real `shelf-sense-ds` components → implement for real in `apps/web` / `apps/api`. Full loop, status conventions, and the backlog-deferral process are in [specs/README.md](specs/README.md).
